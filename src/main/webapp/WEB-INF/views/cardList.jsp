...
<style>
/* ---------- 카드 그리드 ---------- */
#cardGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:50px 40px;
          justify-items:center;max-width:1200px;margin:0 auto}
.item{width:220px;text-align:center}.item img{width:100%;border:1px solid #ddd}

/* ---------- 비교함 ---------- */
#compareBox{position:fixed;right:20px;top:100px;width:160px;padding:10px;
            border:1px solid #ccc;background:#fff;z-index:1010}

/* ---------- 비교 모달 ---------- */
#compareModal{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);
              width:80%;max-width:800px;background:#fff;padding:30px;border-radius:12px;
              box-shadow:0 0 20px rgba(0,0,0,.35);z-index:2000}
...
#modalContent div{width:200px;text-align:center}#modalContent img{width:100%;border:1px solid #ccc}

/* 더보기 */
#loadMoreWrap{text-align:center;margin:40px 0}

/* ---------- 검색바 + 타입버튼 ---------- */
.typeBtn{display:inline-block;padding:7px 20px;margin:0 3px;font-weight:600;
         border:1px solid #aaa;border-radius:9999px;background:#fff;cursor:pointer}
.typeBtn.active{background:#000;color:#fff}
#searchInput{width:260px;padding:8px 35px 8px 35px;font-size:15px;
...
#filterBtn{width:40px;height:40px;margin-left:8px;border:none;border-radius:50%;
           background:#eee;font-size:18px;cursor:pointer;display:inline-flex;
           align-items:center;justify-content:center}

/* ---------- 상세 검색 모달 ---------- */
#advOverlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:3000}
#advModal{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);
          width:90%;max-width:600px;max-height:85vh;background:#fff;border-radius:12px;
          padding:25px;z-index:3001;box-shadow:0 0 20px rgba(0,0,0,.35);
          display:flex;flex-direction:column}
#advModal h3{text-align:center;margin-top:0}
#advKeyword{width:100%;box-sizing:border-box;padding:8px 12px;margin-bottom:15px;
            border:1px solid #ccc;border-radius:8px}
.hot{display:inline-block;padding:4px 12px;margin:3px;border-radius:15px;background:#eee;
     font-size:13px;cursor:pointer}.hot.sel{background:#000;color:#fff}
#modalResultGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));
                 gap:20px;justify-items:center;margin-top:15px;flex:1;overflow-y:auto}
.adv-close{position:absolute;top:15px;right:20px;font-size:22px;font-weight:700;cursor:pointer}
</style>
</head>
<body>
...
window.addEventListener('DOMContentLoaded', () => {
  closeAdv();            // 혹시 열려 있었다면 강제 닫기
  closeCompareModal();   // 비교 모달도 강제 닫기 (필요하면)
